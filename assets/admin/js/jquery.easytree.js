...
//console.log($(n.html()));
        ...
        function getTitleHtml(node) {
            ...
            if (node.href || node.href==0) {
                html += '<input type="' + document.getElementById('inputType').value + '" id="' + node.href + '" ';
				
				var str_arr = node.hrefTarget.split(",");
...
function setIsDropdown()
{
	if(document.getElementsByName('super_category_id'))
	{
		var super_category = document.getElementsByName('super_category_id');
		/*var i = 0;
		for(i=0 ; i < super_category.length ; i++)
		{
			if(super_category[i].checked)
			{
				if(super_category[i].value == 0)
				{
					alert('parent');
				}
			}
		}*/
		if(super_category[0].checked)
		{
			if(super_category[0].value == 0)
			{
				//alert('parent');
				
				//document.getElementById('is_dropdown_req').style.display = 'block';
				$('.is_parent_field').show();
			}
		}
		else
		{
			document.getElementById('is_dropdown2').checked = true;
			document.getElementById('is_outer_menu2').checked = true;
			//document.getElementById('is_dropdown_req').style.display = 'none';
			$('.is_parent_field').hide();
