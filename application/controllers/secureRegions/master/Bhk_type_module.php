...
class Bhk_type_module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('secureRegions/master/Bhk_type_model');
		...
		$this->data['page_module_name'] = 'BHK Type';
		$this->data['table_name'] = 'bhk_type';
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Bhk_type_module/list', $this->data);
		parent::get_footer();
	}
	...
	function list()
	{
		...
		$data_count = $this->Bhk_type_model->get_bhk_type_data($search);
		...
		$this->data['bhk_type_data'] = $this->Bhk_type_model->get_bhk_type_data($search);
	...
	function list_export()
	{
		...
		$this->load->view('secureRegions/master/Bhk_type_module/list_export', $this->data);
	}
	...
	function view($id = "")
	{
		...
		$this->data['bhk_type_data'] = $this->Bhk_type_model->get_bhk_type_data(array("id" => $id));
		if (empty($id)) {
		...
		$this->data['bhk_type_data'] = $this->data['bhk_type_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Bhk_type_module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($id = "")
	{
		...
		if (!empty($id)) {
			$this->data['bhk_type_data'] = $this->Bhk_type_model->get_bhk_type_data(array("id" => $id));
			if (empty($this->data['bhk_type_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Bhk_type_module/edit', $this->data);
		parent::get_footer();
	}
	...
	function do_edit()
	{
		...
		$is_exist = $this->Common_model->get_data(array('select' => '*', 'from' => 'bhk_type', 'where' => "name = \"$name\" and id != $id"));
		...
		if (!empty($id)) {
			...
			$insertStatus = $this->Common_model->update_operation(array('table' => 'bhk_type', 'data' => $enter_data, 'condition' => "id = $id"));
			...
			$insertStatus = $this->Common_model->add_operation(array('table' => 'bhk_type', 'data' => $enter_data));
			if (!empty($insertStatus)) {
	...
	function do_update_status()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_model->update_operation(array('table' => "bhk_type", 'data' => $update_data, 'condition' => "id in ($ids)"));
