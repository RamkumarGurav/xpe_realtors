...
class Property_age_module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('secureRegions/master/Property_age_model');
		...
		$this->data['page_module_name'] = 'Property Age';
		$this->data['table_name'] = 'property_age';
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_age_module/list', $this->data);
		parent::get_footer();
	}
	...
	function list()
	{
		...
		$data_count = $this->Property_age_model->get_property_age_data($search);
		...
		$this->data['property_age_data'] = $this->Property_age_model->get_property_age_data($search);
	...
	function list_export()
	{
		...
		$this->load->view('secureRegions/master/Property_age_module/list_export', $this->data);
	}
	...
	function view($id = "")
	{
		...
		$this->data['property_age_data'] = $this->Property_age_model->get_property_age_data(array("id" => $id));
		if (empty($id)) {
		...
		$this->data['property_age_data'] = $this->data['property_age_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_age_module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($id = "")
	{
		...
		if (!empty($id)) {
			$this->data['property_age_data'] = $this->Property_age_model->get_property_age_data(array("id" => $id));
			if (empty($this->data['property_age_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_age_module/edit', $this->data);
		parent::get_footer();
	}
	...
	function do_edit()
	{
		...
		if (!empty($id)) {
			...
			$insertStatus = $this->Common_model->update_operation(array('table' => 'property_age', 'data' => $enter_data, 'condition' => "id = $id"));
			...
			$insertStatus = $this->Common_model->add_operation(array('table' => 'property_age', 'data' => $enter_data));
			if (!empty($insertStatus)) {
	...
	function do_update_status()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_model->update_operation(array('table' => "property_age", 'data' => $update_data, 'condition' => "id in ($ids)"));
