...
class Designation_module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('secureRegions/master/Designation_model');
		...
		$this->data['table_name'] = 'designation';
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Designation_module/list', $this->data);
		parent::get_footer();
	}
	...
	function list()
	{
		...
		$data_count = $this->Designation_model->get_designation_data($search);
		...
		$this->data['designation_data'] = $this->Designation_model->get_designation_data($search);
	...
	function list_export()
	{
		...
		$this->load->view('secureRegions/master/Designation_module/list_export', $this->data);
	}
	...
	function view($id = "")
	{
		...
		$this->data['designation_data'] = $this->Designation_model->get_designation_data(array("id" => $id));
		if (empty($id)) {
		...
		$this->data['designation_data'] = $this->data['designation_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Designation_module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($id = "")
	{
		...
		if (!empty($id)) {
			$this->data['designation_data'] = $this->Designation_model->get_designation_data(array("id" => $id));
			if (empty($this->data['designation_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Designation_module/edit', $this->data);
		parent::get_footer();
	}
	...
	function do_edit()
	{
		...
		$is_exist = $this->Common_model->get_data(array('select' => '*', 'from' => 'designation', 'where' => "name = \"$name\" and id != $id"));
		...
		if (!empty($id)) {
			...
			$insert_status = $this->Common_model->update_operation(array('table' => 'designation', 'data' => $enter_data, 'condition' => "id = $id"));
			...
			$insert_status = $this->Common_model->add_operation(array('table' => 'designation', 'data' => $enter_data));
			if (!empty($insert_status)) {
	...
	function do_update_status()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_model->update_operation(array('table' => "designation", 'data' => $update_data, 'condition' => "id in ($ids)"));
