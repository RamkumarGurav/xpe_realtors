...
class Property_sub_type_module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('secureRegions/master/Property_sub_type_model');
		...
		$this->data['page_module_name'] = 'Property Sub Type';
		$this->data['table_name'] = 'property_sub_type';
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_sub_type_module/list', $this->data);
		parent::get_footer();
	}
	...
	function list()
	{
		...
		$data_count = $this->Property_sub_type_model->get_property_sub_type_data($search);
		...
		// $this->data['property_sub_type_data'] = $this->Common_model->get_data(array('select' => '*', 'from' => 'property_sub_type', 'where' => "property_type_id > 0", "order_by" => "property_sub_type_name ASC"));
		$this->data['property_sub_type_data'] = $this->Property_sub_type_model->get_property_sub_type_data($search);
	...
	function list_export()
	{
		...
		$this->load->view('secureRegions/master/Property_sub_type_module/list_export', $this->data);
	}
	...
	function view($id = "")
	{
		...
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['property_sub_type_data'] = $this->Property_sub_type_model->get_property_sub_type_data(array("id" => $id));
		if (empty($id)) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. anubhav</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
		...
		$this->data['property_sub_type_data'] = $this->data['property_sub_type_data'][0];

		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_sub_type_module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($id = "")
	{
		...
		if (!empty($id)) {
			$this->data['property_sub_type_data'] = $this->Property_sub_type_model->get_property_sub_type_data(array("id" => $id));
			if (empty($this->data['property_sub_type_data'])) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/master/Property_sub_type_module/edit', $this->data);
		parent::get_footer();
	}
	...
	function do_edit()
	{
		...
		if (empty($_POST['name']) && empty($_POST["property_type_id"])) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. anubhav</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin . $user_access->class_name . "/" . $user_access->function_name);
			exit;
		...
		$is_exist = $this->Common_model->get_data(
			array(
				...
				'where' => "name = \"$name\" and  property_type_id = $property_type_id and id != $id"
			)
		);
		...
		$enter_data['property_type_id'] = $property_type_id;
		...
		if (!empty($id)) {
			...
			$insertStatus = $this->Common_model->update_operation(array('table' => 'property_sub_type', 'data' => $enter_data, 'condition' => "id = $id"));
			...
			$insertStatus = $this->Common_model->add_operation(array('table' => 'property_sub_type', 'data' => $enter_data));
			if (!empty($insertStatus)) {
	...
	function do_update_status()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_model->update_operation(array('table' => "property_sub_type", 'data' => $update_data, 'condition' => "id in ($ids)"));
				if ($response) {
