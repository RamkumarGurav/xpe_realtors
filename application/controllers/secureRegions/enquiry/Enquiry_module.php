...
class Enquiry_module extends Main
{
	...
	function __construct()
	{
		...
		$this->load->model('secureRegions/enquiry/Enquiry_model');
		...
		$this->data['page_module_name'] = 'Enquiry';
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/enquiry/enquiry_module/list', $this->data);
		parent::get_footer();
	}
	...
	function list()
	{
		...
		$data_count = $this->Enquiry_model->get_enquiry_data($search);
		...
		$this->data['enquiry_data'] = $this->Enquiry_model->get_enquiry_data($search);
	...
	function list_export()
	{
		...
		$this->load->view('secureRegions/enquiry/Enquiry_module/list_export', $this->data);
	}
	...
	function view($id = "")
	{
		...
		$this->data['enquiry_data'] = $this->Enquiry_model->get_enquiry_data(array("id" => $id));
		if (empty($id)) {
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/enquiry/Enquiry_module/view', $this->data);
		parent::get_footer();
	}
	...
	function edit($id = "")
	{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('secureRegions/enquiry/Enquiry_module/edit', $this->data);
		parent::get_footer();
	}
	...
	function do_edit()
	{
		...
		if (
			empty($_POST['name']) && empty($_POST['contactno']) && empty($_POST['service']) && empty($_POST['email'])
			&& empty($_POST['message'])
		) {
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" 
			class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> 
		...
		$enter_data['contactno'] = $contactno = trim($_POST['contactno']);
		$enter_data['service'] = $service = trim($_POST['service']);
		$enter_data['email'] = $email = trim($_POST['email']);
		$enter_data['message'] = $message = trim($_POST['message']);
		$enter_data['is_consent'] = $is_consent = 1;
		...
		if (!empty($id)) {
			...
			$insert_status = $this->Common_model->update_operation(array('table' => 'enquiry', 'data' => $enter_data, 'condition' => "id = $id"));
			...
			$insert_status = $this->Common_model->add_operation(array('table' => 'enquiry', 'data' => $enter_data));
			if (!empty($insert_status)) {
	...
	function do_update_status()
	{
		...
		if ($user_access->update_module == 1) {
			...
			if (!empty($id_arr)) {
				...
				$response = $this->Common_model->update_operation(array('table' => "enquiry", 'data' => $update_data, 'condition' => "id in ($ids)"));
